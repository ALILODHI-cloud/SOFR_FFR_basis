"""Build interactive 3M Euribor curve dashboard (frozen reference + evolution overlay)."""
from __future__ import annotations

import json
from pathlib import Path

from analyze_euribor_3m import compute_ecb_meeting_pricing_3m, rebuild_evolution_from_snapshot

ROOT = Path(__file__).resolve().parent
with (ROOT / "euribor_3m_data.json").open(encoding="utf-8") as f:
    data = json.load(f)

data = rebuild_evolution_from_snapshot(data)
(ROOT / "euribor_3m_data.json").write_text(json.dumps(data, indent=2), encoding="utf-8")

if "ecb_meeting_pricing" not in data:
    data["ecb_meeting_pricing"] = compute_ecb_meeting_pricing_3m(
        data["contracts"], data["deposit_facility_pct"], data.get("deposit_facility_as_of")
    )
...
<title>3M Euribor Curve Live</title>
...
.ecb-summary{display:flex;flex-wrap:wrap;gap:10px;margin-bottom:12px}
.ecb-stat{background:#1b2536;border:1px solid var(--line);border-radius:10px;padding:10px 14px;min-width:140px}
.ecb-stat .k{font-size:11px;color:var(--mut)}
.ecb-stat .v{font-size:18px;font-weight:700;margin-top:2px;font-variant-numeric:tabular-nums}
.ecb-chartbox{position:relative;height:220px;margin-bottom:12px}
...
tr.ecb-next td{background:rgba(57,217,138,0.06)}
...
body.view-phone .ecb-chartbox{height:180px}
body.view-phone .ecb-stat{min-width:calc(50% - 6px);flex:1 1 calc(50% - 6px)}
...
@media (max-width: 720px){
  ...
  .ecb-chartbox{height:180px}
  .ecb-stat{min-width:calc(50% - 6px);flex:1 1 calc(50% - 6px)}
...
<div class="wrap">
<header>
  <p style="margin:0 0 10px;font-size:13px"><a href="portal.html" style="color:#93a1b5;text-decoration:none">← Markets portal</a> · <a href="estr_3m_dashboard.html" style="color:#93a1b5;text-decoration:none">3M €STR</a></p>
  <h1>3M Euribor curve · frozen reference + time travel</h1>
...
<div class="card">
  <h2>ECB Governing Council meeting pricing (from 3M Euribor strip)</h2>
...
function yAxisBounds() {
  ...
  vals.push(DATA.deposit_facility_pct);
...
function buildMainChart() {
  ...
  const br = DATA.deposit_facility_pct;
...
function renderHeader(status) {
  ...
  document.getElementById('policyPill').textContent = `ECB dep ${DATA.deposit_facility_pct}%`;
  if (isLiveMode()) document.getElementById('livePill').style.display = 'inline-block';
}

function renderEcbPanel() {
  const ecb = DATA.ecb_meeting_pricing;
  if (!ecb?.meetings?.length) return;

  document.getElementById('ecbNote').textContent = ecb.note || '';

  const sum = document.getElementById('ecbSummary');
  const nxt = ecb.next_meeting;
  sum.innerHTML = `
    <div class="ecb-stat"><div class="k">Deposit facility</div><div class="v">${ecb.deposit_facility_pct.toFixed(2)}%</div></div>
    <div class="ecb-stat"><div class="k">Total easing priced</div><div class="v">${fmtBp(ecb.total_easing_priced_bp)}</div></div>
    ${nxt ? `<div class="ecb-stat"><div class="k">Next: ${nxt.meeting_label}</div><div class="v">${fmtBp(nxt.incremental_bp)}</div><div class="k" style="margin-top:4px">${nxt.cut_pct}% cut · ${nxt.hold_pct}% hold · ${nxt.hike_pct}% hike</div></div>` : ''}`;

  const tbody = document.querySelector('#ecbTbl tbody');
  tbody.innerHTML = '';
  ecb.meetings.forEach(m => {
    const tr = document.createElement('tr');
    if (m.status === 'next') tr.className = 'ecb-next';
    if (m.status === 'past') tr.className = 'ecb-past';
    tr.innerHTML = `
      ...
      <td class="num">${fmtBp(m.cumulative_vs_deposit_bp)}</td>
  ...
  const upcoming = ecb.meetings.filter(m => m.status !== 'past');
  const labels = upcoming.map(m => m.meeting_date.slice(5));
  const cum = upcoming.map(m => m.cumulative_vs_deposit_bp);
  const inc = upcoming.map(m => m.incremental_bp);
  const ctx = document.getElementById('ecbChart');
  if (ecbChart) ecbChart.destroy();
  ecbChart = new Chart(ctx, {
    type: 'bar',
    data: {
      labels,
      datasets: [
...
function renderTable() {
  ...
  [...DATA.contracts].sort((a,b) => a.delivery_ym.localeCompare(b.delivery_ym)).forEach(c => {
    const tr = document.createElement('tr');
    tr.innerHTML = `<td>${c.label}</td><td>${c.symbol}</td><td class="num">${c.implied_rate_pct.toFixed(3)}%</td><td class="num">${fmtBp(c.vs_deposit_bp)}</td><td>${c.latest_date}</td>`;
    tbody.appendChild(tr);
  });
}
...
const VIEW_KEY = 'euribor3m_layout';
...
for name in ("euribor_3m_dashboard.html", "docs/euribor_3m_dashboard.html"):
