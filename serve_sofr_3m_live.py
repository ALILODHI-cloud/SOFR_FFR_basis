#!/usr/bin/env python3
"""Live 3M SOFR curve server — auto-refreshes from Barchart daily."""
...
DATA_FILE = ROOT / "sofr_3m_data.json"
DASHBOARD = ROOT / "sofr_3m_dashboard.html"
PORT = int(os.environ.get("SOFR_3M_PORT", "8791"))
REFRESH_MINUTES = int(os.environ.get("SOFR_3M_REFRESH_MINUTES", "60"))
...
def run_refresh() -> None:
    ...
    print(f"[{utc_str()}] 3M SOFR Barchart refresh…")
    try:
        subprocess.run(
            [sys.executable, str(ROOT / "analyze_sofr_3m.py")],
            cwd=ROOT,
            check=True,
            timeout=3600,
        )
        subprocess.run(
            [sys.executable, str(ROOT / "build_sofr_3m_dashboard.py")],
            cwd=ROOT,
            check=True,
            timeout=120,
        )
...
class Handler(BaseHTTPRequestHandler):
    ...
    def do_GET(self) -> None:
        ...
        if path in ("/", "/index.html", "/sofr_3m_dashboard.html"):
...
def main() -> None:
    ...
    print(f"3M SOFR live dashboard: http://0.0.0.0:{PORT}  (refresh every {REFRESH_MINUTES}m)")
