...
@media(min-width:900px){.cols-4{grid-template-columns:repeat(4,1fr)}.cols-2{grid-template-columns:repeat(2,1fr)}}
...
<div class="grid cols-4">
  <div class="card">
    ...
    <div class="stat blue" id="kSlope"></div>
...
<div class="card">
  <h2>1M SONIA calendar slope &mdash; last <span id="winLbl"></span> sessions</h2>
  <p class="hint">Positive = Dec-27 implied rate above Dec-26 (curve upward between these points). Hover for date &amp; value.</p>
...
document.getElementById('kSlopeSub').textContent =
  `50d mean ${f1(s.slope_mean_50d)} · σ ${s.slope_std_50d.toFixed(1)} bp`;
...
const lineOpts = (yLabel, suggestedMin, suggestedMax) => ({
...
new Chart(document.getElementById('slopeChart'), {
  ...
  options: lineOpts('bp', undefined, undefined),
...
new Chart(document.getElementById('volChart'), {
  ...
  options: lineOpts('bp (ann.)', 0, undefined)
});
