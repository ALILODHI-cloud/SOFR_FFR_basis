...
.pill-portal{border-color:var(--acc);color:var(--acc);background:rgba(57,217,138,.1);font-weight:600}
.navtop{font-size:13px;margin-bottom:14px;display:flex;flex-wrap:wrap;gap:10px;align-items:center}
.navtop a{color:var(--acc2);text-decoration:none}
...
<header>
  ...
  <div style="margin-top:12px">
    ...
    <a class="pill pill-portal" href="portal.html">$1M book · portal →</a>
  </div>
</header>
...
function renderTradeStats() {
  ...
  const cards = [
    ...
    ['Return on margin', f1(T.return_margin_pct) + '%', `~£${(T.margin_ice_gbp||T.margin_assumed_gbp).toLocaleString()} ICE IM`, T.return_margin_pct >= 0 ? 'green' : 'red'],
