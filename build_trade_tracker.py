...
<div class="card" id="legsCard" style="display:none"><h2>Contract levels · vs policy</h2><div id="legsBody"></div></div>
<div class="card"><h2 id="pnlChartTitle">Cumulative P&amp;L</h2><div class="chartbox"><canvas id="pnlChart"></canvas></div></div>
<div class="grid two">
...
function render(D){
  ...
  if(D.trade.market === 'sonia_estr_1m' && D.dual_policy){
    pills.innerHTML = '<span class="pill">'+D.trade.direction+'</span>'
      + '<span class="pill">Bank '+Number(D.dual_policy.bank_rate_pct).toFixed(2)+'%</span>'
      + '<span class="pill">ECB dep '+Number(D.dual_policy.deposit_facility_pct).toFixed(2)+'%</span>'
    ...
    pills.innerHTML = '<span class="pill">'+D.trade.direction+'</span><span class="pill">'+policyLbl+' '+D.bank_rate_pct+'%</span>'
      + (D.trade.entry_locked ? '<span class="pill">Entry locked (EOD)</span>' : '<span class="pill warn">Provisional</span>')
      + '<span class="pill">Updated '+D.generated_utc+'</span>';
  }
  ...
  if(isOutright){
    ...
    const isUkEur = D.trade.market === 'sonia_estr_1m';
    const DP = D.dual_policy || {};
    const entryRel = D.entry.relative_vs_policy_bp != null ? D.entry.relative_vs_policy_bp
      : (D.entry.sonia_vs_bank_bp != null && D.entry.estr_vs_deposit_bp != null
        ? D.entry.sonia_vs_bank_bp - D.entry.estr_vs_deposit_bp : null);
    const markRel = D.mark.relative_vs_policy_bp != null ? D.mark.relative_vs_policy_bp : null;

    document.getElementById('levels').innerHTML = isUkEur ? [
      ['entry','Entry relative (vs policy)',fmtBp(entryRel)],
      ['entry','Entry UK−EUR absolute',fmtBp(L.entry_slope_bp)],
      ['tp','Mark relative',fmtBp(markRel)]
    ].map(([c,l,v])=>'<div class="level '+c+'"><div class="statlbl">'+l+'</div><div class="stat sm neu">'+v+'</div></div>').join('') : [
    ...
    document.getElementById('stats').innerHTML = (isUkEur ? [
      ['P&amp;L', fmtPnl(pnlAmt), cls(pnlAmt)],
      ['Δ relative', fmtBp(entryRel!=null&&markRel!=null ? entryRel-markRel : pnl.slope_bp), cls(pnl.slope_bp)],
      ['Mark relative', fmtBp(markRel), cls(pnl.slope_bp)],
      ['Mark UK−EUR', fmtBp(D.mark.slope_bp), 'neu'],
    ] : [
      ['P&amp;L', fmtPnl(pnlAmt), cls(pnlAmt)],
    ...
    document.getElementById('mainChartTitle').textContent = isUkEur
      ? 'Implied change by end-2026 · relative (UK gap − EUR gap, bp)'
      : D.trade.spread_label + ' spread (bp)';
    document.getElementById('secondChartTitle').textContent = isUkEur
      ? 'Each leg vs own policy (bp)'
      : 'Implied rates (%)';

    if(isUkEur){
      const bank = DP.bank_rate_pct != null ? DP.bank_rate_pct : D.entry.bank_rate_pct;
      const dep = DP.deposit_facility_pct != null ? DP.deposit_facility_pct : D.entry.deposit_facility_pct;
      const ukE = D.entry.short, ukM = D.mark.short, eurE = D.entry.long, eurM = D.mark.long;
      const ukVsE = ukE.vs_bank_bp != null ? ukE.vs_bank_bp : D.entry.sonia_vs_bank_bp;
      const ukVsM = ukM.vs_bank_bp != null ? ukM.vs_bank_bp : D.mark.sonia_vs_bank_bp;
      const eurVsE = eurE.vs_deposit_bp != null ? eurE.vs_deposit_bp : D.entry.estr_vs_deposit_bp;
      const eurVsM = eurM.vs_deposit_bp != null ? eurM.vs_deposit_bp : D.mark.estr_vs_deposit_bp;
      let legsHtml = '<p class="sub" style="margin:0 0 10px">'
        + 'BoE Bank Rate <b>'+Number(bank).toFixed(2)+'%</b>'
        + (DP.bank_rate_as_of ? ' (as of '+DP.bank_rate_as_of+')' : '')
        + ' · ECB deposit <b>'+Number(dep).toFixed(2)+'%</b>'
        + (DP.deposit_facility_as_of ? ' (as of '+DP.deposit_facility_as_of+')' : '')
        + '<br/>Relative = (SONIA Dec-26 − Bank Rate) − (ESTR Dec-26 − deposit). '
        + 'Betting this <b>decreases</b> (further UK underperformance vs EUR vs policy). '
        + '<a href="estr_sonia_1m_dashboard.html" style="color:#4aa8ff">Full 1M ESTR vs SONIA curves →</a></p>'
        + '<table class="tbl"><thead><tr>'
        + '<th>Leg</th><th>Contract</th><th>Entry %</th><th>vs policy</th><th>Mark %</th><th>vs policy</th><th>Δ implied</th>'
        + '</tr></thead><tbody>';
      legsHtml += '<tr><td>Short UK</td><td>'+(ukE.symbol||'JUZ26')+' Dec-26 SONIA</td>'
        +'<td class="num">'+fmtPct(ukE.implied_rate_pct)+'</td><td class="num">'+fmtFedBp(ukVsE)+' vs BR</td>'
        +'<td class="num">'+fmtPct(ukM.implied_rate_pct)+'</td><td class="num">'+fmtFedBp(ukVsM)+' vs BR</td>'
        +'<td class="num">'+((ukM.implied_rate_pct-ukE.implied_rate_pct)*100).toFixed(1)+'</td></tr>';
      legsHtml += '<tr><td>Long EUR</td><td>'+(eurE.symbol||'IJZ26')+' Dec-26 ESTR</td>'
        +'<td class="num">'+fmtPct(eurE.implied_rate_pct)+'</td><td class="num">'+fmtFedBp(eurVsE)+' vs dep</td>'
        +'<td class="num">'+fmtPct(eurM.implied_rate_pct)+'</td><td class="num">'+fmtFedBp(eurVsM)+' vs dep</td>'
        +'<td class="num">'+((eurM.implied_rate_pct-eurE.implied_rate_pct)*100).toFixed(1)+'</td></tr>';
      legsHtml += '<tr><td colspan="3"><b>Relative (UK gap − EUR gap)</b></td>'
        +'<td class="num"><b>'+fmtBp(entryRel)+'</b></td><td></td>'
        +'<td class="num"><b>'+fmtBp(markRel)+'</b></td>'
        +'<td class="num"><b>'+fmtBp(entryRel!=null&&markRel!=null?entryRel-markRel:null)+'</b></td></tr>';
      legsHtml += '<tr><td colspan="3">Absolute UK − EUR</td>'
        +'<td class="num">'+fmtBp(D.entry.slope_bp)+'</td><td></td>'
        +'<td class="num">'+fmtBp(D.mark.slope_bp)+'</td>'
        +'<td class="num">'+fmtBp(pnl.slope_bp)+'</td></tr>';
      legsHtml += '</tbody></table>';
      if(D.view && D.view.note){
        legsHtml += '<p class="sub" style="margin:10px 0 0">'+D.view.note+'</p>';
      }
      document.getElementById('legsCard').style.display='block';
      document.getElementById('legsBody').innerHTML = legsHtml;
    } else {
      const legs = [
        {side:'Long', entry:D.entry.long, mark:D.mark.long},
        {side:'Short', entry:D.entry.short, mark:D.mark.short},
      ];
      let legsHtml = '<p class="sub" style="margin:0 0 10px">'+policyLbl+' '+D.bank_rate_pct+'%'
        + (D.bank_rate_as_of ? ' (as of '+D.bank_rate_as_of+')' : '')
        + ' · quoted spread '+fmtBp(D.entry.slope_bp)+'</p>'
        + '<table class="tbl"><thead><tr><th>Side</th><th>Contract</th><th>Entry</th><th>vs '+policyLbl+'</th><th>Mark</th><th>vs '+policyLbl+'</th><th>Δ bp</th></tr></thead><tbody>';
      legs.forEach(({side, entry, mark})=>{
        const vsEntry = entry.vs_fed_bp != null ? entry.vs_fed_bp : (entry.implied_rate_pct!=null&&D.bank_rate_pct!=null?(entry.implied_rate_pct-D.bank_rate_pct)*100:null);
        const vsMark = mark.vs_fed_bp != null ? mark.vs_fed_bp : (mark.implied_rate_pct!=null&&D.bank_rate_pct!=null?(mark.implied_rate_pct-D.bank_rate_pct)*100:null);
        const dBp = entry && mark ? ((mark.implied_rate_pct - entry.implied_rate_pct)*100).toFixed(1) : '—';
        legsHtml += '<tr><td>'+side+'</td><td>'+(entry.symbol||'')+' '+(entry.label||'')+'</td>'
          +'<td class="num">'+fmtPct(entry.implied_rate_pct)+'</td><td class="num">'+fmtFedBp(vsEntry)+'</td>'
          +'<td class="num">'+fmtPct(mark.implied_rate_pct)+'</td><td class="num">'+fmtFedBp(vsMark)+'</td>'
          +'<td class="num">'+dBp+'</td></tr>';
      });
      legsHtml += '</tbody></table>';
      document.getElementById('legsCard').style.display='block';
      document.getElementById('legsBody').innerHTML = legsHtml;
  ...
  if(isOutright){
    ...
    const isUkEur = D.trade.market === 'sonia_estr_1m';
    const entryRel = D.entry.relative_vs_policy_bp;
    if(isUkEur && path.length && path[0].relative_vs_policy_bp != null){
      const rel = path.map(r=>r.relative_vs_policy_bp);
      const ds = [
        {label:'Relative vs policy (bp)',data:rel,borderColor:'#c084fc',pointRadius:3,borderWidth:2},
        {label:'Entry relative',data:labels.map(()=>entryRel),borderColor:'#4aa8ff',borderDash:[5,4],pointRadius:0},
        {label:'Zero',data:labels.map(()=>0),borderColor:'#64748b',borderDash:[2,3],pointRadius:0,borderWidth:1},
      ];
      new Chart(document.getElementById('mainChart'), { type:'line', data:{labels,datasets:ds},
        options:{responsive:true,maintainAspectRatio:false,plugins:{legend:{labels:{color:'#93a1b5'}}},
          scales:{x:{ticks:{color:'#93a1b5'}},y:{ticks:{color:'#93a1b5'}, title:{display:true,text:'bp',color:'#93a1b5'}}}}
      });
      new Chart(document.getElementById('secondChart'), {
        type:'line',
        data:{labels,datasets:[
          {label:'SONIA Dec-26 vs Bank Rate',data:path.map(r=>r.sonia_vs_bank_bp),borderColor:'#4aa8ff',pointRadius:2,borderWidth:2},
          {label:'ESTR Dec-26 vs deposit',data:path.map(r=>r.estr_vs_deposit_bp),borderColor:'#ffb84a',pointRadius:2,borderWidth:2},
          {label:'Policy (0)',data:labels.map(()=>0),borderColor:'#64748b',borderDash:[2,3],pointRadius:0,borderWidth:1},
        ]},
        options:{responsive:true,maintainAspectRatio:false,plugins:{legend:{labels:{color:'#93a1b5'}}},
          scales:{x:{ticks:{color:'#93a1b5'}},y:{ticks:{color:'#93a1b5'}, title:{display:true,text:'bp vs policy',color:'#93a1b5'}}}}
      });
      document.getElementById('pathHead').innerHTML = '<th>Date</th><th>UK %</th><th>EUR %</th><th>UK vs BR</th><th>EUR vs dep</th><th>Relative</th><th>UK−EUR</th><th>Cum $</th>';
      const pt = document.querySelector('#pathTbl tbody');
      [...path].reverse().forEach(r=>{
        const c = r[cumKey] != null ? r[cumKey] : r.cum_pnl_gbp;
        pt.innerHTML += '<tr><td>'+r.date+'</td>'
          +'<td class="num">'+r.quoted_long_rate.toFixed(3)+'</td>'
          +'<td class="num">'+r.quoted_short_rate.toFixed(3)+'</td>'
          +'<td class="num">'+Number(r.sonia_vs_bank_bp).toFixed(1)+'</td>'
          +'<td class="num">'+Number(r.estr_vs_deposit_bp).toFixed(1)+'</td>'
          +'<td class="num">'+Number(r.relative_vs_policy_bp).toFixed(1)+'</td>'
          +'<td class="num">'+r.slope_bp.toFixed(1)+'</td>'
          +'<td class="num">'+fmtUsd(c)+'</td></tr>';
      });
    } else {
...
fetch('trades_index.json').then(r=>{
  ...
  INDEX.trades.forEach(t=>{
    ...
    grid.innerHTML += '<a class="card" href="'+t.detail_page+'"><h2>'+t.label+'</h2><p>'+t.position+'</p>'
      ...
      + entryRow + markRow
      + '<div class="row"><span>P&amp;L</span><span class="'+pnlCls+'">'+fmtPnl(t)+'</span></div></a>';
  });
...
<div class="grid cols-2">
  ...
  <a class="card" href="euribor_3m_dashboard.html"><h2>3M Euribor curve · ECB</h2><p>17 Sep EOD · Z27−Z26 +43.5 · next Oct ECB</p></a>
...
fetch('trades_index.json').then(r=>r.json()).then(INDEX=>{
  ...
  INDEX.trades.forEach(t=>{
    ...
    el.innerHTML += '<a class="card featured" href="'+t.detail_page+'"><h2>'+t.label+'</h2><p>'+t.position+'</p>'
      ...
      + '<span style="display:block;margin-top:10px;font-size:13px">P&amp;L <span class="'+pnlCls+'">'+pnlDisp+'</span></span></a>';
  });
}).catch(()=>{
