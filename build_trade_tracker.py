...
if(isOutright){
  ...
  const entryLine = labels.map(()=>L.entry_rate_pct);
  const datasets = [{label:'Rate %',data:rates,borderColor:'#39d98a',pointRadius:3}];
  if(L.entry_rate_pct!=null) datasets.push({label:'Entry',data:entryLine,borderColor:'#4aa8ff',borderDash:[5,4],pointRadius:0});
...
def portal_html(trades: list[dict]) -> str:
    ...
    <span style="display:block;margin-top:10px;font-size:13px">P&amp;L <span class="{pnl_cls}">{sign}£{abs(t['pnl_gbp']):,.0f}</span></span>
  </a>"""
        )
    trade_section = "\n".join(cards)
...
def main() -> None:
    ...
    outputs: list[tuple[str, str]] = [
        ("trade_tracker.html", HUB_HTML.replace("__INDEX_JSON__", json.dumps(index))),
        ("portal.html", portal_html(index["trades"])),
    ]

    for t in index["trades"]:
        data = json.loads((ROOT / t["data_file"]).read_text(encoding="utf-8"))
        page = DETAIL_HTML.replace("__PAGE_TITLE__", f"{data['trade']['label']} · live tracker").replace(
