#!/usr/bin/env python3
"""Live 30-day Fed Funds curve server — auto-refreshes from Barchart daily."""
...
DATA_FILE = ROOT / "ff_30d_data.json"
DASHBOARD = ROOT / "ff_30d_dashboard.html"
PORT = int(os.environ.get("FF_30D_PORT", "8793"))
REFRESH_MINUTES = int(os.environ.get("FF_30D_REFRESH_MINUTES", "60"))
...
def run_refresh() -> None:
    ...
    print(f"[{utc_str()}] 30-day Fed Funds Barchart refresh…")
    try:
        subprocess.run(
            [sys.executable, str(ROOT / "analyze_ff_30d.py")],
            cwd=ROOT,
            check=True,
            timeout=3600,
        )
        subprocess.run(
            [sys.executable, str(ROOT / "build_ff_30d_dashboard.py")],
            cwd=ROOT,
            check=True,
            timeout=120,
        )
...
class Handler(BaseHTTPRequestHandler):
    ...
    def do_GET(self) -> None:
        ...
        if path in ("/", "/index.html", "/ff_30d_dashboard.html"):
...
def main() -> None:
    ...
    print(f"30-day Fed Funds live dashboard: http://0.0.0.0:{PORT}  (refresh every {REFRESH_MINUTES}m)")
