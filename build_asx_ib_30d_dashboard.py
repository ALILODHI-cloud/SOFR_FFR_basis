"""Build interactive ASX 30-day cash rate curve dashboard (frozen reference + evolution overlay)."""
from __future__ import annotations

import json
from pathlib import Path

from analyze_asx_ib_30d import compute_rba_meeting_pricing

ROOT = Path(__file__).resolve().parent
with (ROOT / "asx_ib_30d_data.json").open(encoding="utf-8") as f:
    data = json.load(f)

if "rba_meeting_pricing" not in data:
    data["rba_meeting_pricing"] = compute_rba_meeting_pricing(
        data["contracts"], data["cash_rate_pct"], data.get("cash_rate_as_of")
    )
...
<title>ASX 30-Day Cash Rate Curve Live</title>
...
.rba-summary{display:flex;flex-wrap:wrap;gap:10px;margin-bottom:12px}
.rba-stat{background:#1b2536;border:1px solid var(--line);border-radius:10px;padding:10px 14px;min-width:140px}
.rba-stat .k{font-size:11px;color:var(--mut)}
.rba-stat .v{font-size:18px;font-weight:700;margin-top:2px;font-variant-numeric:tabular-nums}
.rba-chartbox{position:relative;height:220px;margin-bottom:12px}
...
tr.rba-next td{background:rgba(57,217,138,0.06)}
tr.rba-past td{color:var(--mut)}
...
body.view-phone .rba-chartbox{height:180px}
body.view-phone .rba-stat{min-width:calc(50% - 6px);flex:1 1 calc(50% - 6px)}
...
@media (max-width: 720px){
  ...
  .rba-chartbox{height:180px}
  .rba-stat{min-width:calc(50% - 6px);flex:1 1 calc(50% - 6px)}
...
<div class="card">
  <h2>RBA Board meeting pricing (from ASX 30-day cash rate strip)</h2>
  <p class="hint" id="rbaNote"></p>
  <div class="rba-summary" id="rbaSummary"></div>
  <div class="rba-chartbox"><canvas id="rbaChart"></canvas></div>
  <div class="tblwrap"><table id="rbaTbl"><thead><tr>
    <th>Meeting</th><th>Ref 1M</th><th>Implied %</th><th>Cum vs cash rate</th><th>Δ at meeting</th><th>Cut</th><th>Hold</th><th>Hike</th><th>Probs</th>
...
<div class="card">
  ...
  <div class="tblwrap"><table id="tbl"><thead><tr>
    <th>Delivery</th><th>Symbol</th><th>Implied %</th><th>vs cash rate</th><th>As of</th>
  </tr></thead><tbody></tbody></table></div>
</div>
...
function updatePinTray() {
  ...
  pins.forEach((pin, i) => {
    ...
    card.innerHTML = `
      ...
      <div class="row"><span>vs cash rate ${DATA.cash_rate_pct}%</span><span>${h ? fmtBp(h.vs_cash_bp) : '—'}</span></div>
          ...
          callbacks: {
            ...
            label: ctx => {
              ...
              if (ctx.datasetIndex === 1 && fp && hm) {
                ...
                lines.push(`vs cash rate: ${fmtBp(hm.vs_cash_bp)}`);
              }
              if (ctx.datasetIndex === 0 && fp) {
                lines.push(`vs cash rate: ${fmtBp(fp.vs_cash_bp)}`);
              }
...
function renderHeader(status) {
  ...
  document.getElementById('policyPill').textContent = `RBA ${DATA.cash_rate_pct}%`;
...
function renderMpcPanel() {
  ...
  const sum = document.getElementById('rbaSummary');
  const nxt = rba.next_meeting;
  sum.innerHTML = `
    <div class="rba-stat"><div class="k">Cash rate</div><div class="v">${rba.cash_rate_pct.toFixed(2)}%</div></div>
    <div class="rba-stat"><div class="k">Total easing priced</div><div class="v">${fmtBp(rba.total_easing_priced_bp)}</div></div>
    ${nxt ? `<div class="rba-stat"><div class="k">Next: ${nxt.meeting_label}</div><div class="v">${fmtBp(nxt.incremental_bp)}</div><div class="k" style="margin-top:4px">${nxt.cut_pct}% cut · ${nxt.hold_pct}% hold · ${nxt.hike_pct}% hike</div></div>` : ''}`;

  const tbody = document.querySelector('#rbaTbl tbody');
  tbody.innerHTML = '';
  rba.meetings.forEach(m => {
    const tr = document.createElement('tr');
    if (m.status === 'next') tr.className = 'rba-next';
    if (m.status === 'past') tr.className = 'rba-past';
    tr.innerHTML = `
      ...
      <td class="num">${fmtBp(m.cumulative_vs_cash_bp)}</td>
  ...
  const upcoming = rba.meetings.filter(m => m.status !== 'past');
  const labels = upcoming.map(m => m.meeting_date.slice(5));
  const cum = upcoming.map(m => m.cumulative_vs_cash_bp);
  const inc = upcoming.map(m => m.incremental_bp);
  const ctx = document.getElementById('rbaChart');
  if (rbaChart) rbaChart.destroy();
  rbaChart = new Chart(ctx, {
    type: 'bar',
    data: {
      labels,
      datasets: [
        {
          label: 'Cumulative vs cash rate (bp)',
          data: cum,
          type: 'line',
          borderColor: '#4aa8ff',
...
function renderTable() {
  ...
  [...DATA.contracts].sort((a,b) => a.delivery_ym.localeCompare(b.delivery_ym)).forEach(c => {
    const tr = document.createElement('tr');
    tr.innerHTML = `<td>${c.label}</td><td>${c.symbol}</td><td class="num">${c.implied_rate_pct.toFixed(3)}%</td><td class="num">${fmtBp(c.vs_cash_bp)}</td><td>${c.latest_date}</td>`;
    tbody.appendChild(tr);
  });
}
...
const VIEW_KEY = 'asxib30d_layout';
...
for name in ("asx_ib_30d_dashboard.html", "docs/asx_ib_30d_dashboard.html"):
