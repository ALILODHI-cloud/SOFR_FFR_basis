#!/usr/bin/env python3
"""Live ASX 30-day cash rate curve server — auto-refreshes from Barchart daily."""
...
DATA_FILE = ROOT / "asx_ib_30d_data.json"
DASHBOARD = ROOT / "asx_ib_30d_dashboard.html"
PORT = int(os.environ.get("ASX_IB_30D_PORT", "8792"))
REFRESH_MINUTES = int(os.environ.get("ASX_IB_30D_REFRESH_MINUTES", "60"))
...
def run_refresh() -> None:
    ...
    print(f"[{utc_str()}] ASX 30-day cash rate Barchart refresh…")
    try:
        subprocess.run(
            [sys.executable, str(ROOT / "analyze_asx_ib_30d.py")],
            cwd=ROOT,
            check=True,
            timeout=3600,
        )
        subprocess.run(
            [sys.executable, str(ROOT / "build_asx_ib_30d_dashboard.py")],
            cwd=ROOT,
            check=True,
            timeout=120,
        )
...
class Handler(BaseHTTPRequestHandler):
    ...
    def do_GET(self) -> None:
        ...
        if path in ("/", "/index.html", "/asx_ib_30d_dashboard.html"):
...
def main() -> None:
    ...
    print(f"ASX 30-day cash rate live dashboard: http://0.0.0.0:{PORT}  (refresh every {REFRESH_MINUTES}m)")
